...
// CORRECCIÓN CRÍTICA: Agregamos 'User' al final de esta lista para que no falle la línea 77
import { X, Printer, ShieldCheck, MapPin, Phone, Mail, User } from 'lucide-react';

const QuotationPrintModal = ({ isOpen, onClose, data }) => {
  // 1. SEGURIDAD: Si no está abierto o no hay data, no renderizamos nada
  if (!isOpen || !data) return null;

  // 2. VALORES POR DEFECTO (Para evitar errores si falta información)
  const { 
    id = 'BORRADOR', 
          ...
          <div className="bg-white shadow-xl mx-auto w-full max-w-[21cm] min-h-[29.7cm] p-8 sm:p-12 relative text-gray-800 flex flex-col">
            ...
            <div className="bg-blue-50/50 p-6 rounded-xl mb-8 border border-blue-100">
              <h3 className="text-xs font-bold text-[#1e1b4b] uppercase mb-2 flex items-center gap-2">
                {/* ESTA ES LA LÍNEA QUE FALLABA (77): Ahora 'User' ya está importado arriba */}
              ...
              <table className="w-full mb-8 border-collapse">
                ...
                <tbody className="divide-y divide-gray-200">
                  {items.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan="2" className="py-4 text-center text-gray-400 text-sm">Sin ítems registrados</td>
                    </tr>
                  )}
                </tbody>
              </table>
