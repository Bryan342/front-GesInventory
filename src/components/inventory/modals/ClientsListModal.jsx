...
import { X, Search, FileText, Calendar, User, Printer } from 'lucide-react';
...
const ClientsListModal = ({ isOpen, onClose }) => {
  ...
  // --- CARGAR RESERVAS DE SESIÓN ---
  useEffect(() => {
    if (isOpen) {
      // Usamos sessionStorage
      const tempDocs = JSON.parse(sessionStorage.getItem('jordy_temp_docs') || '[]');
      const quotes = tempDocs.filter(d => d.type === 'COTIZACION');
      ...
      <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center backdrop-blur-sm animate-fade-in">
        <div className="bg-white rounded-3xl p-8 w-full max-w-4xl shadow-2xl relative h-[80vh] flex flex-col">
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-500 text-sm">Bandeja de cotizaciones temporales (Sesión Actual).</p>
          </div>
                ...
                {filteredList.length > 0 ? (
                  filteredList.map((res, idx) => (
                    <tr key={idx} className="border-b border-gray-50 hover:bg-indigo-50/30 transition h-16 group">
                      <td className="pl-4">
                        <span className="font-mono font-bold text-indigo-700 bg-indigo-50 px-2 py-1 rounded-md border border-indigo-100">{res.code}</span>
                      </td>
                      <td>
                      ...
                      <td className="text-center font-medium">
                        {res.items ? res.items.length : 0} prod.
                      </td>
                      <td className="text-center">
                        <span className="bg-yellow-100 text-yellow-700 border border-yellow-200 px-3 py-1 rounded-full text-xs font-bold shadow-sm">PENDIENTE</span>
                      </td>
                      <td className="text-right pr-4">
                        <button 
                          onClick={() => setSelectedDoc({
                            ...
                            date: res.date,
                            items: res.items,
                            totalItems: res.items ? res.items.length : 0
                          })}
                  ...
                  <tr>
                    <td colSpan="6" className="text-center py-20">
                      <div className="flex flex-col items-center opacity-40">
                        ...
                        <p>No hay reservas activas en esta sesión.</p>
                      </div>
                    </td>
                  </tr>
                )}
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 flex justify-end"><div className="text-xs text-gray-400">Sesión Actual</div></div>
        </div>
      </div>
